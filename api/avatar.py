...
def get_current_zone():
    ...
    for attempt in range(3):  # extra retry for robustness
        try:
...
def avatar():
    ...
    frames = []
    bg_colors = [(0, 0, 0), (10, 0, 0), (20, 0, 0), (10, 0, 0)]

    zone_short = zone[:10] + '..' if len(zone) > 10 else zone

    for i in range(4):  # 4 frames for smoother loop
        ...
        if i % 2 == 0:
            ...
            draw.text((4, 24), zone_short, fill=(255, 255, 255), font=font)
        else:
            mins = 60 - datetime.now().minute
            draw.text((4, 8), f"{mins}M", fill=(255, 215, 0), font=font)
            draw.text((4, 24), "NEXT", fill=(220, 220, 150), font=font)

        # Flame dots
        draw.point([(10 + i*2, 52), (14 + i*3, 50), (18 + i*2, 54)], fill=(255, 100, 0))
